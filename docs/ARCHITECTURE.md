...
| sharp (dev only) | `npm run slice-logo` bootstrap tool, `npm run build-og` social card |
...
public/
  ...
  assets/og-image.png     Link-preview card (generated by scripts/build-og-image.mjs)
  ...
  assets/showcase/        The three panel artworks (webp; `my`/`dark-side` have alpha),
                          plus `my-mouth` / `my-mouth-shut` — the two strips the
...
scripts/build-og-image.mjs
                          Composes assets/og-image.png from the starburst layers
```
...
a placeholder `tint`, optional `media` and IT/EN title + optional description, plus an
optional per-locale `year` override (the last slot reads OGGI / TODAY). One ruler tick +
year + card per entry; years may repeat (two 2016 slots, three 2018 ones) since order, not
the number, drives the layout. The 15 entries carry the artist's own copy. Any card
that has media opens a full-screen panel on the whole set (the comic is five pages); an
`.mp4`/`.webm` src plays as a video there. The card's face is the first media item when
that is an image, otherwise the card keeps its tint and wears a play mark. An empty list
opens a blank panel, holding the place of artwork to come — that is 1994, waiting for its
...
artwork; the open shot is registered onto it (SIFT + RANSAC homography — the camera moved
between the two frames) and both are then cut down the same hand-measured column to the
two strips the page scales, `my-mouth.webp` (open) and `my-mouth-shut.webp`. The script
prints where that column sits, for `mouth.box` in `panels.ts`. The timeline photos are the
plain end of it: the comic pages pass through at their scan size, the workshop sign is
cropped square on the sign (the cards are near-square and cover-cropped). Then: an
ink-density alpha for the DSOTM shape,
...
`BaseLayout` emits Open Graph + Twitter card tags so chat apps and social sites unfurl a
preview. All URLs are absolute (built off `site` + `base`) and `og:image:width/height` are
declared — WhatsApp falls back to a tiny thumbnail without them. Title and description come
from the page's own props, falling back to the locale's `meta` strings.

`npm run build-og` writes `public/assets/og-image.png`: the 1200x630 card, which is just the
starburst-TM badge centred on white. It reuses the same two transparent layers as
`Logo.astro`, so replacing those and re-running keeps the card in sync with the mark.
